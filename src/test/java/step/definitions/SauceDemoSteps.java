...
public class SauceDemoSteps {
	...
	public SauceDemoSteps(DriverContext driverContext) {
	...
	public void user_want_to_purchase_the_sauce_lab_merchandise_product_on_the_offical_cart_site() {
		driverContext.getDriver().get("https://www.saucedemo.com/");
		driverContext.getDriver().manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
	}
	...
	public void regitered_user_able_to_login_with_valid_user_credentials_and(String username, String password) {
		driverContext.getDriver().findElement(By.id("user-name")).sendKeys(username);
		driverContext.getDriver().findElement(By.id("password")).sendKeys(password);
		driverContext.getDriver().findElement(By.id("login-button")).click();
	}
	...
	public void user_want_to_purchase_product_and_add_to_the_cart(String productName) {
		List<WebElement> elements = driverContext.getDriver().findElements(By.cssSelector("div.inventory_item_name"));
		String idValue = "add-to-cart-"+productName.toLowerCase().replace(" ", "-");
		for (WebElement element : elements) {
			if(element.getText().equals(productName)) {
				driverContext.getDriver().findElement(By.id(idValue)).click();
				break;
			}
		}
	}
	...
	public void user_confirm_the_added_product_and_checkout_the_product_to_purchase_with_details(DataTable dataTable) {
		driverContext.getDriver().findElement(By.className("shopping_cart_link")).click();
		driverContext.getDriver().findElement(By.id("checkout")).click();
	    List<WebElement> elements = driverContext.getDriver().findElements(By.cssSelector(".checkout_info > div > input"));
		...
		driverContext.getDriver().findElement(By.id("continue")).click();
	}
	...
	public void user_able_to_validate_and_confirm_the_added_product_is_correct_one_or_not(DataTable dataTable) {
	    ...
	    for (Map<String, String> map : asMaps) {
			assertEquals(driverContext.getDriver().findElement(By.className("inventory_item_name")).getText(), map.get("productName"));
		}
	}
